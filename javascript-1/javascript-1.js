...
let compareNums = (num1, num2) => num1 > num2 ? num1 : num1 < num2 ? num2 : num1
...
for(let i = 0; i < lettersToPair.length; i++){
    for(let a = 0; a < lettersToPair.length; a++){
        if(lettersToPair[i] === lettersToPair[a] && i !== a && !processedLetters.includes(a)){        
            pairsArray.push([i, a])
            processedLetters.push(i)
        }
    }
}
...
//CODE HERE
  let phone1 = new Phone("Nokia", "Phonio", 6, "Fucia", false)

  let phone2 = new Phone("Apple", "Iphone", 7, "Gold", false)
  
  let phone3 = new Phone("Samsung", "Turdpile Note 13", 1, "Stupid", false)
